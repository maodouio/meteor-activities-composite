...
Router.map(function() {
  ...
  this.route('activitiesIndex', {
    ...
    data: function () {
      return {
        activities: Activities.find({}, {sort: {status: -1, createdAt: -1}}),
  ...
  this.route('/pay', {
    action: function() {
      ...
      var openid;
      Meteor.call("getOpenidByCodeForPay", code, function(error, result) {
        if (error) {
        ...
        if (result) {
          ...
          Meteor.call("createChargeWx", openid, state, function(error, result) {
            if (error) {
              console.log("error", error);
            }
            if (result) {
